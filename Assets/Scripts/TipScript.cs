...
public class TipScript:MonoBehaviour
{
    ...
    string[][] tips = {
        new string[]{ "Move:A&D", "<color=#058923>Kill all the enemy to win this game!</color>", "Press Spacebar to shoot & kill the enemy." },
        new string[]{ "Touch the green platform to change color.","Shoot green bullets on the green enemy to kill it." },
        new string[]{ "Drop an eraser (press X) on the line to remove it.","Enemy`s bullets can change platform`s color.","Change your color to green and kill the green enemy." ,"Draw a line as a ramp","Shoot a bullet onto the ramp to kill the enemy", "Kill black enemy ② by trapping its movement ","Now kill the enemy" }
    };
    Vector3[][] poses = { 
        new Vector3[]{new Vector3(30.6f, 80.6f, 0), new Vector3(30.6f, 80.6f, 0), new Vector3(30.6f, 80.6f, 0) },
